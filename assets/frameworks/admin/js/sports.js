$(document).ready(function () {
    ...
    refresh_datatable_sports();

    $('.datepicker_sports').datepicker({
        format: 'dd-mm-yyyy',
    ...
    $(this).closest('.more_less_class').find('.Readless').css('display', 'none');
    $(this).closest('.more_less_class').find('.Readmore').css('display', 'block');
});
$(document).on('click', '.readless_button', function() {
    $(this).closest('.more_less_class').find('.Readless').css('display', 'block');
    $(this).closest('.more_less_class').find('.Readmore').css('display', 'none');
});
});

function refresh_datatable_sports() {
    if ($.fn.DataTable.isDataTable('#datatable-sports-event')) {
        $('#datatable-sports-event').DataTable().destroy();
    }
    // datatables
    table_category = $('#datatable-sports-event').DataTable({
        ...
        "ajax": {
            "url": base_url+"admin/sports/ajax_list",
            "type": "POST",
            "data": function (data) {
        ...
        initComplete: function () {
            this.api().columns([0]).every(function () {
                ...
                inputTitle = $('#datatable-sports-event thead th').eq(column.index()).text();
...
$('body').on('click', '.sport_event_delete_confirmation', function(e) {
    e.preventDefault();
    $('#sport_event_id').val($(this).attr('data-id'));
});

$('body').on('click', '#sport-event-delete-save', function(e) {
    e.preventDefault();
    var sport_event_id = $('#sport_event_id').val();
    console.log(sport_event_id);
    $.ajax({
        type: "post",
        url: base_url+"admin/sports/delete",
        data: {sport_event_id: sport_event_id},
        dataType: "json",
        success: function (data) {
            if(data.status == 1){
                $('#sport_event_deleted').modal('hide');
                alert(data.message);
                refresh_datatable_sports();
            }else{
                $('#sport_event_deleted').modal('hide');
