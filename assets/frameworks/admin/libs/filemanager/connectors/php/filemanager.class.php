...
class Filemanager {
    ...
    protected $thumbnail_width = 270;
    protected $thumbnail_height = 247;
    ...
    public function add() {
        ...
        $target_dir = "plugins/filemanager/userfiles/";
        $target_file = $target_dir . basename($_FILES["newfile"]["name"]);
        $ext = pathinfo($target_file, PATHINFO_EXTENSION);
        $mime = mime_content_type($current_path . $_FILES["newfile"]["name"]);
        $filesize = $_FILES['newfile']['size'];

        $query = "INSERT INTO `assets`(`type`, `mime`, `extension`, `size`, `description`, `path`, `user_id`, `created`, `modified`) "
                . "VALUES ('post','" . $mime . "','" . $ext . "','" . $filesize . "','" . $_FILES["newfile"]["name"] . "','" . $target_file . "','" . $_SESSION['user_id'] . "','" . $date . "','" . $date . "')";
        $link->query($query);
        // automatically resize image if it's too big
